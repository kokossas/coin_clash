# /home/ubuntu/coin_clash/core/engine.py
...
from .repositories import PlayerRepo, CharacterRepo, MatchRepo, EventRepo, ItemRepo
from .config_loader import load_config
from .scenario_loader import load_scenarios, EVENT_TYPE_TO_CATEGORY
from .utils import SeedableRandom
from .exceptions import InsufficientParticipantsError, CriticalMatchError, SkipEvent
...
class MatchEngine:
    ...
    def _select_participants(self, count: int, source_pool: Dict[int, Character]) -> List[Character]:
        ...
        if not source_pool or len(source_pool) < count:
            from .exceptions import SkipEvent
...
# Example of how to potentially use this (would be in the main script)
# def simulate_match():
#     config = load_config()
...
#     # ... code to create/load players ...
#     players = player_repo.get_all_players(limit=config['num_players_default'])
#     if len(players) < config['num_players_min']:
#         # Create more players
#         pass 
...
#         start_method='cap', # Or 'timeout'
#         start_threshold=config['num_players_default'] # Or timeout seconds
#     )

#     # 3. Simulate Character Purchases
#     match_participants = []
#     # ... code for players to 'buy' characters, pay fee, assign names ...
#     # Assign characters to the match_id
#     # char_repo.assign_character_to_match(char.id, match.id)

#     # 4. Init & Run Engine
#     engine = MatchEngine(match.id, config, scenarios, player_repo, char_repo, match_repo, event_repo, item_repo, random_seed=123)
#     winner, match_log = engine.run_match(match_participants)
